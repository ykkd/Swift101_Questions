...
// 新しい構造体「Post」を定義し、その中に「userId」(Int型)「id」(Int型)「title」(String型)「body」(String型)というプロパティを追加してください。

/*
 関数「fetchPosts(completion:)」を定義し、APIエンドポイント「"https://jsonplaceholder.typicode.com/posts"」からすべての投稿を取得してください。
 この関数は非同期で動作し、投稿が取得できた場合にはResult型の[Post]オブジェクトを、エラーが発生した場合にはResult型のErrorオブジェクトをcompletionハンドラに渡してください。
 */
